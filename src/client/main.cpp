...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 2: // register业务
        {
            ...
            sem_wait(&rwsem); // 等待信号量， 子线程处理完注册消息会通知这里
        }
        break;
        case 3: // quit业务
            close(clientfd);
...
void doRegResponse(json &responsejs)
{
    ...
    {
        cerr << "name is already exist, register fail!" << endl;
    }
    else // 注册成功
    {
...
unordered_map<string, string> commandMap = {
    ...
    {"groupchat", "群聊，格式groupchat:groupid:groupmessage"},
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning)
    {
        ...
        if (-1 == idx)
        {
            command = commandbuf; // 运行help
        }
        else
        {
            command = commandbuf.substr(0, idx); // 取命令处理，如chat， addfriend
        }
        ...
        // 调用相应命令的时间处理回调，mainMeanu对修改封闭，添加新功能不需要修改该函数
...
void creategroup(int clientfd, string str)
{
    ...
    if (-1 == idx)
    {
        cout << "creategroup command invalid!" << endl;
        return;
    }
    ...
    if (-1 == len)
    {
        cout << "send creategroup msg error ->" << buffer << endl;
    }
}
// "addgroup" command handler
